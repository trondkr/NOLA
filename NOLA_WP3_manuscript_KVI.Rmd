...
for(year in years){
  ...
  for(x in 1:3){
   ...
   stat_summary(geom="ribbon", fun.min = function(x) quantile(x, 0.25), fun.max = function(x) quantile(x, 0.75), fill = "light grey") + 
    ...
    theme_minimal() +
    theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
    ggtitle(paste0(year,": ", run_name_short[x]))
...
```{r relative_particle_depth, fig.cap = particle_depth, fig.height = 4, fig.width = 7}
...
for(year in years){
  ...
  for(x in 1:3){
    ...
    ggplot(aes(x=date, y=relative_depth))+ 
   # Plot mean + confidence interval:
    # stat_summary(geom="ribbon", fun.data=mean_cl_normal, fill="grey")+
    # stat_summary(geom="line", fun=mean)+
   # Plot median + upper/lower quantiles:
   stat_summary(geom="ribbon", fun.min = function(x) quantile(x, 0.25), fun.max = function(x) quantile(x, 0.75), fill = "light grey") + 
    ...
    if(x == 1){
    plot_dep <- plot_dep + ylab("Relative particle depth") 
    }
