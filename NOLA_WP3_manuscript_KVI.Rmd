...
# NOT WORKING:
# test<-raster("C:/Users/KVI/OneDrive - NIVA/SharkMove_local/Analyses/data/gis/GEBCO_29_Oct_2024/gebco_2024_n71.8506_s66.1816_w8.2178_e22.8955.tif")
# saveRDS(test,"C:/Users/KVI/OneDrive - NIVA/SharkMove_local/Analyses/data/gis/GEBCO_29_Oct_2024/GEBCO_29_Oct_2024.rds", version = 2)
# 
# options(ggOceanMaps.userpath = "C:/Users/KVI/OneDrive - NIVA/SharkMove_local/Analyses/data/gis/GEBCO_29_Oct_2024/")
# 
# ggOceanMaps::basemap(limits = c(17.4, 19.25,69.25, 69.7), bathymetry = T, land.col = landcol, legends = TRUE, bathy.style = "rub") 
...
study_area <- zoomed + annotation_custom(ggplotGrob(overview), xmin = 17.28, xmax = 18.0, 
                       ymin = 69.22, ymax = 69.4)
...
for(year in years){
  ...
  for(x in 1:2){
    ...
    newdat_firstday$firstday <- predict.gam(mod_firstday, newdata = newdat_firstday) 
    ...
    # Asterix if significant
    # if(summary(mod_firstday)$s.table[4]<0.05){
    #   ast <- "*"
    # } else { ast <- ""}
    ...
    if(year == years[1] & x ==1){
       plot_x <- plot_x +
           ggtitle(label=paste0("A. ",year), subtitle = paste0(run_name_short[x],ast))
             } else if (year == years[2] & x ==1) {
        plot_x <- plot_x +
           ggtitle(label=paste0("B. ",year), subtitle = paste0(run_name_short[x],ast))
        } else if (year == years[1] & x ==2) {
        plot_x <- plot_x +
       ggtitle(paste0("E. ",year), subtitle = paste0(run_name_short[x],ast))
     }  else {
      plot_x <- plot_x +
       ggtitle(paste0("F. ",year), subtitle = paste0(run_name_short[x],ast))
     } 
    ...
    newdat_residence$residence <- predict.gam(mod_residence, newdata = newdat_residence) 
    
    # Asterix if significant
    # if(summary(mod_residence)$s.table[4]<0.05){
    #   ast <- "*"
    # } else { ast <- ""}
    ...
    if(year == years[1] & x ==1){
       plot_x <- plot_x +
           ggtitle(label=paste0("C. ",year), subtitle = paste0(run_name_short[x],ast))
             } else if (year == years[2] & x ==1) {
        plot_x <- plot_x +
           ggtitle(label=paste0("D. ",year), subtitle = paste0(run_name_short[x],ast))
        } else if (year == years[1] & x ==2) {
        plot_x <- plot_x +
       ggtitle(paste0("G. ",year), subtitle = paste0(run_name_short[x],ast))
     }  else {
      plot_x <- plot_x +
       ggtitle(paste0("H. ",year), subtitle = paste0(run_name_short[x],ast))
     } 
...
time_and_residence_mods <- grid.arrange(grobs=plotlist, layout_matrix = matrix(c(1:8),ncol=2,byrow=FALSE), bottom=textGrob("Volume flux (log(m3/day))"))

ggsave(time_and_residence_mods, file="../Figures/time_and_residence_mods.jpg", width = 180, height = 200, units = "mm", dpi = 300)
