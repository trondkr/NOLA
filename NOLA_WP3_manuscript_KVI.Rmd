...
title: "Seasonal and interannual variation in riverine influence in a sub-Arctic fjord"
author:
  - Kristina Ø. Kvile:
...
institute:
  ...
  - NINA: Norwegian Institute  for Nature Research, Tromsø, Norway
abstract: "**Abstract**: *Introduction:* Climate change effects on riverine input in coastal ecosystems.Nutrients that can influence coastal productivity, and particulate and dissolved material that can cause coastal darkening. *Method*: Particle tracking runs to simulate material originating from the river (dissolved or particulate), following the fate of material released at the Målselv river mouth into the Malangen fjord and eventually entering the Norwegian Sea. *Results*: Seasonal and interannual variation in residence time of riverin material, spatial extent of riverin material. Differences between dissolved (neutral) and particulate (sinking) material. *Discussion*: Inputs from rivers to coast, potential ecological effects (light and nutrient availability, stratification). Our results highlight that strong seasonality in residence time and distribution of riverine material will shape the potential for both near-coast processing, removal and biological uptake of riverine organic matter and nutrients; as well as the potential for river-derived material to reshape light, OM and nutrient availability in waters further offshore."
editor_options: 
...
study_area <- figs(name="study_area","Study area. The background map shows the location of the study area (black rectangle), and the inset shows the details of the study area. The black points show the particle release area at the mouth of the Målselv river. The colored boxes and dashed line show the limits of subareas used to quantify the distribution of riverin material (Outside: the Norwegian Sea outside the Outer box).")

particle_release <- figs(name="particle_release","Number of particles released per day (A), the cumulative number of particles during the run (B), and the river volume flux per date for 2018 (C) and 2019 (D). The released particles (A) were scaled by the river volume flux (C, D) to represent the temporal variation in the amount of riverine material. Panels A and B are illustrated for one year, but the setup is the same for both years.")

particle_depth <- figs(name="particle_depth","Depth distribution of particles per day of the simulation (counting from 1st of February) throughout the study area for the different simulations. The output from the two simulated years are combined. The median and relative depth (depth of the particle as percentage of the total water column depth, were 0 is at the surface and 100 at the sea floor) are shown as black lines, with 25 % and 75 % percentiles indicated as grey shading.")

num_particles <- figs(name="num_particles","Cumulative distribution of riverine material during the simulations (counting all particles that have passed through a grid cell, with particles weighted by river water volume flux at the day of release). The top panels show the full distribution and the lower panels a zoomed-in view of the Malangen system (indicated with a black rectangle in the upper panel). Runs for two years with neutrally buoyant or sinking particles are compared. Note that the color scale has been truncated to a maximum of 15 000 for all panels to facilitate visualization and comparison (the maximum value for a single grid cell is indicated in the title).")

num_particles_day <- figs(name="num_particles_day","Distribution of riverine material in the study area at the first day of every 2nd month during the simulation (counting all particles that are in a grid cell that day, weighting particles by river water volume flux at the day of release). Runs with neutrally buoyant and sinking particles are compared (different columns). Note that the color scale has been truncated to a maximum of 100 to allow comparison and visualization (the maximum value for a single for a single grid cell is indicated per panel).")
...
outside_box_lon = c(5, 18.76)
outside_box_lat = c(69.571701, 74)
...
## Aims
For a northern Norwegian case study site (Malangen), use high-resolution particle-tracking modeling to study the seasonality of river outflow and residence time of river material in the fjord to better understand the potential effect of seasonality and high flow events for fjord biogeochemistry and coastal darkening.  
        

# Methods

## Study area
The study area encompasses the mouth of the Målselv river in Northern Norway and the downstream Malangen fjord that can transport riverine material out into the Norwegian Sea (`r figs("study_area", display = "cite")`). This river fjord system has been the subject of previous observational studies focusing on organic matter and nutrient dynamics along the freshwater-marine continuum, providing valuable background data for the current study (e.g. Frigstad et al. 2020, Schultze et al. 2022). The study area was divided into five sub-areas at varying distances from the river mouth to study the flux and residence time of riverine material.  
...
zoomed <- maparea_all +
  ...
  geom_text(aes(x = min(outer_box_lon), y = max(outer_box_lat-0.02), label = "Outer box"),size = 4, vjust = 0, hjust = 0) +
   geom_rect(aes(xmin = min(outside_box_lon), xmax = max(outside_box_lon), ymin = min(outside_box_lat), ymax = max(outside_box_lat)), 
...
To investigate the spatial distribution and residence time of material originating from the Målselv river in the downstream Malangen fjord, we coupled an ocean circulation model to a Lagrangian particle tracking model where the simulated particles represent different types of riverine material. We simulated two different types of particles, representing dissolved and particulate material, respectively. Dissolved material was modeled by specifying that particles are neutrally buoyant, i.e., following the vertical water movement in the ocean model, while particulate material was modeled by specifying that particles are negatively buoyant, i.e., being influenced by the vertical water movement but simultaneously sinking towards the seafloor. We used sinking speeds of 0.3 or 0.8 mm/s, representing typical silt and sand particles with grain sizes of  0.05 and 0.2 mm, respectively, consistent with similar values observed in fjords and estuaries (Uncles et al. 2010, Meslard et al. 2018).

### Particle tracking model
To model particle drift with ocean currents, we used the open-source software OpenDrift [@Dagestad2018]. OpenDrift is a framework for modeling the trajectories and fate of objects or substances drifting in the ocean, where different models can be used depending on the application. We used the basic OceanDrift module for tracking water masses or passive tracers. OpenDrift is available at github.com/OpenDrift/opendrift and the specific setup used here is available at github.com/trondkr/NOLA.  

The particles (both neutrally buoyant and sinking) are influenced by vertical mixing, i.e., they move vertically according to eddy diffusivity and buoyancy, with vertical turbulent mixing being parameterized from wind speed (Sundby, 1983) (*the vertical diffusivity  included in the Norkyst files included some unrealistic values*). Additionally, the particles move vertically according to vertical ocean currents (vertical advection) (*has minimal impact compared to vertical turbulent mixing and could probably be ignored)*. We set the horizontal diffusivity in the simulations to 10 m ^2^/s (ref?). The resuspension threshold for sinking particles was set to a bottom current speed of 0.15 m/s, as shear stress is not included in the available forcing data. If particles hit the seafloor, they are moved back to the previous location, meaning that they can continue to move if the bottom current moves in a different direction at the next time step (we also tested a setup where particles were deactivated after hitting the seafloor, but then the simulation would halt after few timesteps). Similarly, particles that hit land during the simulation move back to their previous location in the ocean. We used a time step of 1 hour during the simulations. 

### Ocean circulation model
The particle tracking model was coupled offline to archived hydrodynamical data from a fjord model covering Malangen and the adjacent fjords and coastal areas using 160m x 160m resolution and 35 vertical levels. The free-surface, terrain-following, primitive equations ocean model ROMS [Regional Ocean Modeling System, http://myroms.org, @Shchepetkin2005, @Haidvogel2008] was applied to reproduce the fjord dynamics and hydrography. The circulation model was initialized on the 1st of March 2017 by model data from the Norwegian Coastal Model system NorKyst800 [presented in detail and validated by @Asplin2020 and operational data accessible at https://thredds.met.no provided by the Norwegian Meteorological Institute, MET-Norway]. Also, open boundary conditions were retrieved from NorKyst800. The atmospheric forcing was provided from AROME MetCoOp (Meteorological Co-operation on Operational Numerical Weather Prediction) 2.5km, the main forecasting system at the MET-Norway [@Muller2017]. Tides from TPXO7.2 global tidal analysis were included [@Egbert2002], and the daily river flow rates used were based on measurements from the NVE, i.e., the same river runoff data as applied in the scaling of the simulated particles. An identical model set up for a Norwegian fjord system further south (Hardangerfjorden) is presented in detail and validated in [@Dalsøren2020].  


### Simulations
We released particles in a band from 18.538°E/69.308°N to 18.506°E/69.306°N representing a cross-section of the river mouth of the Målselv river (`r figs("study_area", display = "cite")`). Initially, the particles were released at the sea surface (0 m depth). We started the particle drift simulation on the 1st of February for each of two years (2018, 2019) and followed the drift of particles until the end of the same year (31st of December, i.e., each year was simulated separately). We released 100 particles per day from the 1^st^ of February until the end of September, i.e., a total of 24 200 particles per year (`r figs("particle_release", display = "cite")`).   

To get a more realistic representation of the seasonal variation in outflow of river material, we scaled the particles by the total river water volume flux at the release day (vertically integrated mass transport, in m^3-s^), provided by the Norwegian Water Resources and Energy Directorate (NVE). I.e., particles released at a specific day were weighted by the volume flux at that day, divided by the total number of particles released that day. In that way, particles released during the spring and summer flood period represent a larger amount of material than during periods of lower volume flux, even though the number of particles released remain the same (`r figs("particle_release", display = "cite")`).  
...
* Divide into material remaining in the upper surface layer (0-10 m), which will have the largest biological impacts in the water column, and water staying close to the sea floor (can be considered as settled out). 
...
`r figs("particle_depth", display = "cite")` shows the depth distribution of particles in the different runs (neutral vs. 0.3 mm/s sinking speed). Although sinking particles quickly reach a deeper mean depth than neutrally buoyant particles, sinking particles tend to stay in the upper 100 m and are never deactivated.  
...
grid.arrange(grobs = plotlist, ncol = 3, as.table = TRUE)  
