...
# Map layers
maparea_all <- get_stamenmap(bbox = c(left = 15.7, bottom = 69.2, right = 21.8,
                                      top = 70.5), 
...
years <- c(2018,2019)
runs<-c("neutralParticles","sinkingParticles_deactivated_-0.001ms")
...
fullRaster <- raster(resolution = reso, xmn=15.7, xmx=21.8, ymn=69.2, ymx=70.5)
...
To get a more detailed view of the seasonal variation in water flow, we scaled the particles by the total river water volume flux at the release day (vertically integrated mass transport, in m^3-day^ divided by 1000 - *unit*?, *source?*). I.e., the particles released at a specific day was weighted by the volume flux at that day, divided by the total number of particles released that day, so that the particles represent the volume flux. In that way, particles released during the spring and summer flood period gets more weight in the analyses (`r figs("particle_release", display = "cite")`).  
...
To model particle drift with ocean currents, we used the open source trajectory modelling framework OpenDrift [@Dagestad2018] which was coupled offline to the NorFjords160 ocean model reanalysis with 160 m horizontal spatial resolution... *more info on NorFjords160 here*...  

We applied two different setups of the particle drift model, one setup where particles represent dissolved material and are neutrally buoyant (i.e. following the vertical movement of water) and one setup where particles represent particulate material and are negatively buoyant (i.e. being influenced by the vertical movement of water but also sinking towards the seafloor with a certain fixed sinking speed). For the sinking particles, we tested sinking speeds of 0.3, 0.5 and 0.8 mm/s, representing mud and sand particles with grain size 0.05-0.2 mm (references). The the resuspension threshold for sinking particles was set to 0.15 m/s (references).   
...
# grid.arrange(grobs = plotlist, layout_matrix = matrix(c(1:8),nrow=4,byrow=FALSE))
...
plot_legend <- ggplot(data=counts_i, aes(x=Date,y=n,fill=Month)) +
                 ...
                 xlim(xlims) + ylim(ylims)+
                ggtitle(paste0(year," ",i," box"))  +
                 theme_minimal() + theme(legend.direction = "horizontal",
...
grid.arrange(grobs=plotlist, layout_matrix = matrix(c(1:8),ncol=4,byrow=TRUE),
            left=textGrob("Number of particles", rot=90))
